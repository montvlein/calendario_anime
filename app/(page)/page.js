            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    { Array.from({ length: mostRecentAnime }, (v, i) => (
                        <tr key={i} className={`border-b border-gray-200 dark:border-gray-700 ${i%2 ? 'opacity-90' : ''}`}>
                            {dateView.map((date, index) => (
...
function EventCard({ event }) {
    return (
        <a
            href={event.url}
            className="group flex flex-col justify-center place-items-center text-xs leading-5 rounded-lg overflow-hidden relative"
        >
            <img src={event.cover} alt={event.title} className="w-full object-cover rounded-lg w-[200px] h-[140px]" />
            ...
            <p className="absolute bottom-10 left-1/2 transform -translate-x-1/2 translate-y-full
                ...
                {event.title} - <span className="text-gray-300">cap. {event.chapter}</span>
            </p>
        </a>
    )
}
